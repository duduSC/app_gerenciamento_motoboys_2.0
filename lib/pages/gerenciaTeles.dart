...
import 'package:app_gerenciamento_motoboys/services/tabelaPreco.dart';
...
class _GerenciamentoTelesPageState extends State<GerenciamentoTelesPage> {
  ...
  Future<void> _adicionarTele(String motoboyId, String enderecoDestino) async {
    ...
    try {
      final double distanciaKm = await _mapsService.getDistance(
        _enderecoOrigem.trim(),
        enderecoDestino.trim(),
      );

      final double valor = _tabelaPrecosService.calcularValorDaTele(
        distanciaKm,
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Tele adicionada com sucesso!"),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text("Gerenciar Teles")),
      body: FutureBuilder<List<Motoboy>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return Center(
              child: Text("Erro ao carregar motoboys: ${snapshot.error}"),
            );
          }
